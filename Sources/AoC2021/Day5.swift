...
enum Day5: Day {
    ...
    struct Line: Equatable {
        ...
        var start: Point { .init(x1, y1) }
        var end: Point { .init(x2, y2) }

        var isVertical: Bool { x1 == x2 }
        var isHorizontal: Bool { y1 == y2 }
        var isHorizontalOrVertical: Bool { isHorizontal || isVertical }
        var lineDirection: Int? {
            if isHorizontal {
                return x1 <= x2 ? 1 : -1
            } else if isVertical {
                return y1 <= y2 ? 1 : -1
        ...
        fileprivate static let parser = Int.parser()
            .skip(",".utf8)
            .take(Int.parser())
            .skip(" -> ".utf8)
            .take(Int.parser())
    ...
    struct Grid: Equatable {
        ...
        var onGridData: [Point: Int] {
            lines.reduce(into: [:]) { result, line in
                for point in line.hvOnGridPoints {
        ...
        fileprivate static let parser = Many(Line.parser, separator: "\n".utf8)
            .map(Grid.init(lines:))
    }
