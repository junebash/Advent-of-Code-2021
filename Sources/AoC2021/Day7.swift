...
private let positionParser = Many(Int.parser(), separator: ",".utf8)
...
enum Day7: Day {
    ...
    struct Candidate {
        var position: Int = .min
        var fuelUse: Int = .max

        func minimumFuelUse(comparedTo other: Candidate) -> Candidate {
            if other.fuelUse < fuelUse {
                return other
            } else {
                return self
            }
    ...
    struct CrabSubmarineSet {
        ...
        func misunderstoodMinimumFuelUseCandidate() -> Candidate {
            let (min, max) = positionCounts.keys.minMax()!
            return (min...max).reduce(Candidate()) { candidate, position in
                candidate.minimumFuelUse(comparedTo: Candidate(
                    position: position,
                    fuelUse: misunderstoodFuelUse(for: position)
                ))
            }
        }

        func actualMinimumFuelUseCandidate() -> Candidate {
            let (min, max) = positionCounts.keys.minMax()!
            return (min...max).reduce(Candidate()) { candidate, position in
                candidate.minimumFuelUse(comparedTo: Candidate(
                    position: position,
                    fuelUse: actualFuelUse(for: position)
                ))
            }
        }

        private func misunderstoodFuelUse(for targetPosition: Int) -> Int {
            positionCounts.reduce(into: 0) { totalFuelUse, positionCount in
                let (position, count) = positionCount
                let singleFuelUse = abs(targetPosition - position)
                totalFuelUse += singleFuelUse * count
            }
        }

        private func actualFuelUse(for targetPosition: Int) -> Int {
            positionCounts.reduce(into: 0) { totalFuelUse, positionCount in
                let (position, count) = positionCount
                let distance = abs(targetPosition - position)
                let singleFuelUse = FuelUse.compute(for: distance)
                totalFuelUse += singleFuelUse * count
            }
        }
    }
    ...
    enum FuelUse {
        private static var lookupTable: [Int: Int] = [0: 0]

        static func compute(for distance: Int) -> Int {
            if let fuelUse = lookupTable[distance] {
                return fuelUse
            }
            let fuelUse = distance + compute(for: distance - 1)
            lookupTable[distance] = fuelUse
            return fuelUse
        }
    }

    static func solution1() throws -> Int {
        try CrabSubmarineSet(
            counts: positionParser.parse(input)
                .orThrow("Couldn't parse crab submarine positions")
                .reduce(into: [Int: Int]()) { counts, position in
                    counts[position, default: 0] += 1
                }
        ).misunderstoodMinimumFuelUseCandidate().fuelUse
    }
