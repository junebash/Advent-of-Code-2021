...
private enum DigitSegment: Hashable {
    case a, b, c, d, e, f, g
...
private struct Digit: Hashable {
    var segments: Set<DigitSegment>

    static let zero: Digit = .init(segments: [.a, .b, .c, .e, .f, .g])
    static let one: Digit = .init(segments: [.c, .f]) // 2 segments
    static let two: Digit = .init(segments: [.a, .c, .d, .e, .g])
    static let three: Digit = .init(segments: [.a, .c, .d, .f, .g])
    static let four: Digit = .init(segments: [.b, .c, .d, .f]) // 4 segments
    static let five: Digit = .init(segments: [.a, .b, .d, .f, .g])
    static let six: Digit = .init(segments: [.a, .b, .d, .e, .f, .g])
    static let seven: Digit = .init(segments: [.a, .c, .f]) // 3 segments
    static let eight: Digit = .init(segments: [.a, .b, .c, .d, .e, .f, .g]) // 7 segments
    static let nine: Digit = .init(segments: [.a, .b, .c, .d, .f, .g])

    init(segments: Set<DigitSegment>) {
    ...
    init?<S: StringProtocol>(string: S) {
        ...
        for character in string {
            guard let segment = DigitSegment(character: character) else { return nil }
...
private struct Display {
    let signalPatterns: Set<Digit>
    let output: [Digit]

    private var mapping: [Digit: Digit] = [:]

    init(patterns: Set<Digit>, output: [Digit]) {
        self.signalPatterns = patterns
        self.output = output

        computeMapping()
    }

    private mutating func computeMapping() {
        guard
            let one = signalPatterns.first(where: { $0.segments.count == 2 }),
            let four = signalPatterns.first(where: { $0.segments.count == 4 }),
            let seven = signalPatterns.first(where: { $0.segments.count == 3 }),
            let eight = signalPatterns.first(where: { $0.segments.count == 7 })
        else { return }
        var patterns = signalPatterns
        [one, four, seven, eight].forEach { patterns.remove($0) }
        mapping[one] = .one
        mapping[four] = .four
        mapping[seven] = .seven
        mapping[eight] = .eight
        ...
        // find 6
        guard let six = patterns.first(where: { digit in
            digit.segments.count == 6 && one.segments.contains(where: { !digit.segments.contains($0) })
        }) else { return }
        patterns.remove(six)
        mapping[six] = .six

        // find zero
//        guard let zero = patterns.first(where: { digit in
//            digit.segments.count == 6 && digit.segments
//        })
...
private let digitParser = Prefix(while: \.isLetter).utf8
    .compactMap(Digit.init(string:))
private let displayParser = Many(digitParser, atLeast: 10, atMost: 10, separator: " ".utf8)
    .map(Set.init)
    .skip(" | ".utf8)
    .take(Many(digitParser, atLeast: 4, atMost: 4, separator: " ".utf8))
    .map(Display.init)
private let inputParser = Many(displayParser, separator: "\n".utf8)


private let uniqueSegmentCounts: Set<Int> = [2, 3, 4, 7]
...
enum Day8: Day {
    ...
    static func solution1() throws -> Int {
        try inputParser.parse(input)
            .orThrow("Failed to parse lines")
            .reduce(0) { currentCount, display in
                currentCount + display.output.count(where: { digit in
                    uniqueSegmentCounts.contains(digit.segments.count)
                })
            }
    }

    static func solution2() throws -> Int {
        let _ = try inputParser.parse(input)
            .orThrow("Failed to parse lines")
        return 0
    }
